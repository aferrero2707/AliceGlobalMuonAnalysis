...
void PlotDEResidualvsMomentum(THnSparse* hnx, THnSparse* hny, int deId, TCanvas& c)
{
  ...
  for (int ci = 0; ci < 2; ci++) {
    auto coordinate = coordinates[ci];
    // select DE
    hn[ci]->GetAxis(deAxisId)->SetRange(deIndex + 1, deIndex + 1);

    // residual[coordinate][sign][quadrant][momentum]
    std::array<std::array<std::vector<double>, 4>, 2> momentum;
    std::array<std::array<std::vector<double>, 4>, 2> momentumErr;
    std::array<std::array<std::vector<double>, 4>, 2> residual;
    std::array<std::array<std::vector<double>, 4>, 2> residualErr;

    TMultiGraph* mg = new TMultiGraph();
    mg->SetTitle(Form("DE%d #Delta(%s) vs. momentum;p (GeV/c); #Delta(%s) (cm)", deId, coordinate.c_str(), coordinate.c_str()));
    TGraphErrors* gr = new TGraphErrors();
    gr->AddPoint(0, 1000);
    gr->AddPoint(hn[ci]->GetAxis(pAxisId)->GetXmax(), 1000);
    mg->Add(gr,"lp");
    ...
    for (int sign = 0; sign < 2; sign++) {
      ...
      for (int quadrant = 0; quadrant < 4; quadrant++) {
        ...
        for (int mom = 0; mom < hn[ci]->GetAxis(pAxisId)->GetNbins(); mom += momRebin) {
          ...
          proj->SetTitle(std::format("DE{}({}), Q{} {}", deId, coordinate, quadrant, (sign == 0 ? "+" : "-")).c_str());
          ...
          residual[sign][quadrant].push_back(result.first);
          residualErr[sign][quadrant].push_back(result.second);
      ...
      for (int quadrant = 0; quadrant < 4; quadrant++) {
        if (residual[sign][quadrant].empty()) {
          std::cout << std::format("DE{} Q{} ({}) d{}[0]=NA",
              deId, quadrant, (sign == 0 ? "+" : "-"), coordinate) << std::endl;
          continue;
        }
        std::cout << std::format("DE{} Q{} ({}) d{}[0]={}",
            deId, quadrant, (sign == 0 ? "+" : "-"), coordinate, residual[sign][quadrant][0]) << std::endl;
        TGraphErrors* gr = new TGraphErrors(momentum[sign][quadrant].size(),
            momentum[sign][quadrant].data(),
            residual[sign][quadrant].data(),
            momentumErr[sign][quadrant].data(),
            residualErr[sign][quadrant].data());
        gr->SetLineColor(colors[quadrant]);
        gr->SetMarkerColor(colors[quadrant]);
        gr->SetMarkerStyle(markers[sign]);
        gr->SetMarkerSize(2);
...
void PlotDEResidual(THnSparse* hnx, THnSparse* hny, int deId, TCanvas& c)
{
  if (hnx->GetNdimensions() < 5) {
    return;
  }
  ...
  for (int ci = 0; ci < 2; ci++) {
    ...
    // select DE and momentum
    hn[ci]->GetAxis(0)->SetRange(0, -1);
    hn[ci]->GetAxis(1)->SetRange(0, -1);
    hn[ci]->GetAxis(2)->SetRange(0, -1);
    hn[ci]->GetAxis(3)->SetRange(0, -1);
    hn[ci]->GetAxis(deAxisId)->SetRange(deIndex + 1, deIndex + 1);
    if (pAxisId >= 0) {
      // p >= xx GeV/c
      int momBin = hn[ci]->GetAxis(pAxisId)->FindBin(mchMomMin + epsilon);
      int momBinMax = hn[ci]->GetAxis(pAxisId)->GetNbins();
      hn[ci]->GetAxis(pAxisId)->SetRange(momBin, momBinMax);
    }

    // residual[coordinate][sign][momentum]
    std::array<std::vector<double>, 2> momentum;
    std::array<std::vector<double>, 2> momentumErr;
    std::array<std::vector<double>, 2> residual;
    std::array<std::vector<double>, 2> residualErr;
    ...
    for (int sign = 0; sign < 2; sign++) {
      ...
      auto result = PlotDXY(proj, c, pdfFileName);
      if (std::isnan(result.first) || std::isnan(result.second)) {
        continue;
      }

      double avgMom = 1;
      momentum[sign].push_back(avgMom);
      momentumErr[sign].push_back(0);
      residual[sign].push_back(result.first);
      residualErr[sign].push_back(result.second);

      delete proj;

      if (residual[sign].empty()) {
        std::cout << std::format("DE{} ({}) d{}[0]=NA",
            deId, (sign == 0 ? "+" : "-"), coordinate) << std::endl;
        continue;
      }
      std::cout << std::format("DE{} ({}) d{}[0]={}",
          deId, (sign == 0 ? "+" : "-"), coordinate, residual[sign][0]) << std::endl;
      TGraphErrors* gr = new TGraphErrors(momentum[sign].size(),
          momentum[sign].data(),
          residual[sign].data(),
          momentumErr[sign].data(),
          residualErr[sign].data());
      gr->SetLineColor(colors[0]);
      gr->SetMarkerColor(colors[0]);
      gr->SetMarkerStyle(markers[sign]);
      gr->SetMarkerSize(2);
      gr->SetLineStyle(lineStyles[sign]);
      mg->Add(gr,"pl");
      auto* entry = legend->AddEntry(gr, (sign == 0 ? " (+)" : " (-)"), "P");
      entry->SetTextColor(colors[0]);
    }
...
void muonGlobalAlignmentDEResiduals(int deId = 500, const char* _rootFileName = "AnalysisResults.root")
{
  fAnalysisResults = new TFile(_rootFileName);
  pdfFileName = std::format("deResiduals-{}.pdf", deId);
